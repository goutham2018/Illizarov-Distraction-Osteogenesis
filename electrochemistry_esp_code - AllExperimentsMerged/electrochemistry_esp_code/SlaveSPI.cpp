...
esp_err_t SlaveSPI::begin(gpio_num_t so, gpio_num_t si, gpio_num_t sclk, gpio_num_t ss, size_t buffer_size, int (*callback)()) {
    ...
    tx_buffer       = (uint8_t *)heap_caps_malloc(_max(max_buffer_size, 32), SPI_MALLOC_CAP);
    rx_buffer       = (uint8_t *)heap_caps_malloc(_max(max_buffer_size, 32), SPI_MALLOC_CAP);
    ...
    */
    ...
    gpio_set_pull_mode(so,   GPIO_PULLUP_ONLY);
...
/*** FREE SPI HOST **********/
esp_err_t SlaveSPI::stop(void)
{
	esp_err_t err = spi_slave_free(spi_host);  // Free the SPI module
//    if (err != ESP_OK) {
//        DEBUG_PRINT(err);
        return err;
//    }
...
void SlaveSPI::callbackAfterTransmission(spi_slave_transaction_t * trans) {
	...
	/*Serial.print("Rx: ");
	Serial.print(((char *)transaction->rx_buffer)[0],HEX);Serial.print(",");
	Serial.print(((char *)transaction->rx_buffer)[1],HEX);Serial.print(",");
	Serial.print(((char *)transaction->rx_buffer)[2],HEX);Serial.print(",");
	Serial.print(((char *)transaction->rx_buffer)[3],HEX);Serial.print(",");
	Serial.print(((char *)transaction->rx_buffer)[4],HEX);Serial.print(",");
	Serial.print(((char *)transaction->rx_buffer)[5],HEX);Serial.print(",");
	Serial.print(((char *)transaction->rx_buffer)[6],HEX);Serial.println();*/
...
esp_err_t SlaveSPI::initTransmissionQueue() {
    ...
    int size = _min(max_buffer_size, output_stream.length());                  // NOT over the buffer's size.
   // memcpy((void *)transaction->tx_buffer, output_stream.getBuffer(), size);  // Rearrange the tx data.
    memcpy((void *)transaction->tx_buffer, array1, 64);
    output_stream.remove(0, size);                                            // Segmentation. Remain for future.
	/*Serial.print("Tx: ");
	Serial.print(((char *)transaction->tx_buffer)[0],HEX);Serial.print(",");
	Serial.print(((char *)transaction->tx_buffer)[1],HEX);Serial.print(",");
	Serial.print(((char *)transaction->tx_buffer)[2],HEX);Serial.print(",");
	Serial.print(((char *)transaction->tx_buffer)[3],HEX);Serial.print(",");
	Serial.print(((char *)transaction->tx_buffer)[4],HEX);Serial.print(",");
	Serial.print(((char *)transaction->tx_buffer)[5],HEX);Serial.print(",");
	Serial.print(((char *)transaction->tx_buffer)[6],HEX);Serial.println();*/
...
//    for(int i = 0; i < 1; i++)
//{
//  Serial.println(output_stream[i]);
//}
}
